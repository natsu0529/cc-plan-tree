...
3. Run `cc-plan-tree render .cc-plan-tree/plan.json --format png --out <path>` and report the saved file path to the user. If the `cc-plan-tree` command is not found, retry with `uvx cc-plan-tree render ...`; if uv is also unavailable, tell the user to run `pip install cc-plan-tree`.
